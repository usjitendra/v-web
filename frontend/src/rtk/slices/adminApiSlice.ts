...
export const adminApi = createApi({
 ...
 tagTypes: ["Sellers", "Buyers", "Batches", "EmailTypes", "EmailUsers", "EmailMessages"],

    endpoints: (builder) => ({

        /* ---------------- GET SELLERS ---------------- */
        getSellers: builder.query<SellerResponse, { page?: number; limit?: number }>({
        ...
        /* ---------------- GET ADMIN BATCHES ---------------- */
        getAdminBatches: builder.query<AdminBatchResponse, { page?: number; limit?: number }>({
            query: ({ page = 1, limit = 10 }) => {
                const params = new URLSearchParams({
                    page: String(page),
                    limit: String(limit),
...
createEmailType: builder.mutation<EmailTypeItem, { type_name: string; is_active?: boolean }>({
    query: (body) => ({ url: `/email/types`, method: "POST", data: body }),
    invalidatesTags: ["EmailTypes"],
}),

updateEmailType: builder.mutation<EmailTypeItem, { id: number; data: Partial<EmailTypeItem> }>({
    query: ({ id, data }) => ({ url: `/email/types/${id}`, method: "PUT", data }),
    invalidatesTags: ["EmailTypes"],
}),

deleteEmailType: builder.mutation<{ success: boolean; message: string }, number>({
...
getEmailUsers: builder.query<EmailUserResponse, number>({
    query: (typeId) => ({ url: `/email/types/${typeId}/users`, method: "GET" }),
    providesTags: ["EmailUsers"],
}),

createEmailUser: builder.mutation<EmailUserItem, { typeId: number; email: string; is_active?: boolean }>({
    query: ({ typeId, ...body }) => ({ url: `/email/types/${typeId}/users`, method: "POST", data: body }),
    invalidatesTags: ["EmailUsers"],
}),

updateEmailUser: builder.mutation<EmailUserItem, { userId: number; data: Partial<EmailUserItem> }>({
    query: ({ userId, data }) => ({ url: `/email/users/${userId}`, method: "PUT", data }),
    invalidatesTags: ["EmailUsers"],
}),

deleteEmailUser: builder.mutation<{ success: boolean; message: string }, number>({
...
getEmailMessages: builder.query<EmailMessageResponse, number>({
    query: (userId) => ({ url: `/email/users/${userId}/messages`, method: "GET" }),
    providesTags: ["EmailMessages"],
}),

createEmailMessage: builder.mutation<EmailMessageItem, { userId: number; subject: string; body: string; is_active?: boolean }>({
    query: ({ userId, ...body }) => ({ url: `/email/users/${userId}/messages`, method: "POST", data: body }),
    invalidatesTags: ["EmailMessages"],
}),

updateEmailMessage: builder.mutation<EmailMessageItem, { msgId: number; data: Partial<EmailMessageItem> }>({
    query: ({ msgId, data }) => ({ url: `/email/messages/${msgId}`, method: "PUT", data }),
    invalidatesTags: ["EmailMessages"],
}),

deleteEmailMessage: builder.mutation<{ success: boolean; message: string }, number>({
